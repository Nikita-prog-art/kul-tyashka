...
DARK_NAMES = [
    ("Жаждущих", "кинжалов"), ("Ненасытной", "утробы"), ("Заскорузлых", "пирамид"),
    ("Упёртых", "баранов"), ("Вечной", "войны"), ("Пылающих", "костров"),
    ("Гнилых", "топей"), ("Слепых", "богов"), ("Голодных", "псов"),
    ("Кровавых", "жатв"), ("Вопящих", "могил"), ("Павших", "курганов"),
    ("Багровых", "теней"), ("Чёрных", "исповедален"), ("Мёртвых", "колыбелей"),
    ("Склепов", "проклятий"), ("Костяных", "тронов"), ("Оскаленных", "черепов"),
]
MILD_NAMES = [
    ("Барабанов", "войны"), ("Стальных", "мечей"), ("Древних", "царств"),
    ("Тысячи", "речек"), ("Сухих", "степей"), ("Гремящих", "копыт"),
    ("Высоких", "башен"), ("Сторожевых", "костров"), ("Железных", "дорог"),
]
GOOD_NAMES = [
    ("Звенящих", "ручьёв"), ("Парящего", "лотоса"), ("Ясных", "зорь"),
    ("Тихих", "вод"), ("Плодородных", "нив"), ("Светлых", "родников"),
    ("Утренней", "росы"), ("Бессмертных", "журавлей"), ("Благого", "пути"),
    ("Праведной", "зари"), ("Чистых", "помыслов"), ("Мирных", "садов"),
    ("Незамутнённых", "источников"), ("Мудрых", "наставников"),
]

EMPEROR_NAMES = ["Тяньцан", "Хэлань", "Чаншэн", "Юаньмин", "Сюаньи", "Чжэньюань",
                 "Линлун", "Байцзэ", "Цзиньхай", "Куньлунь", "Хуанцюань", "Бицзяо"]
SUPERCONTINENT_NAMES = ["Цзюйфэн", "Ланьцан", "Ханьюэ", "Цинлянь", "Тайсюань",
                        "Чанлю", "Хуанлун", "Юньчжун", "Сяншань", "Чжаохуэй"]
...
def roll_supercontinent():
    """Возвращает None или словарь с описанием суперконтинента."""
    if _rng.randrange(100000) < 2:  # 0,002% — великий суперконтинент
        if roll_die(2) == 1:
            return {"тип": "великий суперконтинент", "название": "Му",
                    "император": "Ци Шихуанди", "горы": "Южные Краесветные горы",
                    "источник": "подтверждено"}
        return {"тип": "великий суперконтинент", "название": "???",
                "император": "Ци ???", "горы": "Северные Краесветные горы",
                "источник": "подтверждено"}
    if _rng.randrange(50000) == 0:  # ~0,002% — обычный суперконтинент
        n = roll_die(200)
        if n == 1:
            return {"тип": "суперконтинент", "название": "Бабилония",
                    "император": "Ци Семирамида", "горы": None, "источник": "подтверждено"}
        if n == 2:
            return {"тип": "суперконтинент", "название": "Шемаха",
                    "император": "Ци Шемаха", "горы": None, "источник": "подтверждено"}
        emp = _rng.choice(EMPEROR_NAMES)
        cont = _rng.choice(SUPERCONTINENT_NAMES)
        return {"тип": "суперконтинент", "название": cont, "император": f"Ци {emp}",
                "горы": None, "источник": "сгенерировано"}
    return None


def generate_continent_name(morality: str) -> str:
    m = morality
    if m in ("падший", "обречённый"):
        pool = DARK_NAMES
    elif m == "демонический":
        pool = DARK_NAMES + DARK_NAMES + MILD_NAMES
    elif m == "сражающийся":
        pool = MILD_NAMES + DARK_NAMES
    elif m == "ортодоксальный":
        pool = GOOD_NAMES + MILD_NAMES
    else:
        pool = GOOD_NAMES
    a, b = _rng.choice(pool)
    name = f"континент {a} {b}"
    # Падшие континенты у края карты иногда получают «громкие» имена во славу Вечных Императоров
    return name
...
def print_continent(parsed, supercont):
    ...
    if supercont:
        ...
        print("  (правила для суперконтинентов в тексте не раскрыты — множители считаются как у обычного)")
...
def build_parser():
    ...
    q.add_argument("--name", help="название континента (иначе — интерактивный вопрос)")
    q.add_argument("--save", action="store_true", help="сохранить <континент>/continent.json")
    q.add_argument("--no-super", action="store_true", help="не бросать на суперконтинент")
    ...
    q.add_argument("--continent-name", help="название нового континента (иначе — интерактивный вопрос)")
    q.add_argument("--name"); q.add_argument("--continent"); q.add_argument("--gg", action="store_true")
...
КУЛЬТЯШКА — оцифровка механик. Быстрый обзор:
  ...
  continent generate|parse|list генерация/разбор континентов (номер 1d10000000); имя задаёт пользователь
  chance                        «шанс один на миллиард/миллион»
...
def _continent_flow(number, save, no_super, name=None):
    ...
    supercont = None if no_super else roll_supercontinent()
    suggestion = generate_continent_name(parsed["характеристики"]["мораль"]["значение"])
    if supercont:
        if supercont["тип"] == "великий суперконтинент":
            suggestion = f"{supercont['тип']} {supercont['название']}"
        else:
            suggestion = f"{supercont['тип']} {supercont['название']}"
    # Сначала показываем значения и множители — название задаёт пользователь
    print_continent(parsed, supercont)
    if name:
        final_name = name
    elif sys.stdin.isatty():
        final_name = input(f"Название континента [Enter — «{suggestion}»]: ").strip() or suggestion
    else:
